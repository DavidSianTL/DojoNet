...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function init() {
      ...
      if (filtroAccion) filtroAccion.addEventListener("input", debounce(aplicarFiltros, 300))
    ...
    function aplicarFiltros() {
      const accion = filtroAccion ? filtroAccion.value.toLowerCase() : ""
      ...
      dayGroups.forEach((dayGroup) => {
        ...
        timelineItems.forEach((item) => {
          const itemAccion = item.querySelector(".timeline-action").textContent.toLowerCase()
          ...
          if (accion && !itemAccion.includes(accion)) mostrarItem = false
          if (usuario && !itemUsuario.includes(usuario)) mostrarItem = false
    ...
    function exportarExcel() {
      ...
      ;["ID", "Fecha", "Hora", "Acción", "Descripción", "Usuario", "Sistema"].forEach((text) => {
        ...
        if (dayGroup.style.display !== "none") {
          ...
          timelineItems.forEach((item) => {
            if (item.style.display !== "none") {
              ...
              tdUsuario.textContent = item.querySelector(".timeline-user").textContent.replace("Usuario: ", "")
              row.appendChild(tdUsuario)
  
              // Sistema
              const tdSistema = document.createElement("td")
              tdSistema.textContent = item.querySelector(".timeline-system").textContent.replace("Sistema: ", "")
