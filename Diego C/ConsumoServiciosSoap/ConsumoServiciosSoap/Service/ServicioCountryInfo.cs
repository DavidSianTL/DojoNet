    ...
    public class ServicioCountryInfo: ICountryInfoService
    {
        ...
        public async Task<List<string>> ListOfLanguagesByName()
        {
            try
            {
                ...
                return resultado.Body.ListOfLanguagesByNameResult
                    .Select(lang => $"{lang.sName} ({lang.sISOCode})")
                    .ToList();
            }
            catch (Exception ex)
            {
                return new List<string> { $"Error al consultar los idiomas: {ex.Message}" };
            }
        }
        public async Task<List<string>> ListOfLanguagesByCode()
        {
            try
            {
                ...
                // Realizamos la llamada al servicio ListOfLanguagesByCode
                var resultado = await cliente.ListOfLanguagesByCodeAsync();

                // Cerramos el cliente después de recibir la respuesta
                await cliente.CloseAsync();

                // Procesamos y retornamos la lista de idiomas con su código
                return resultado.Body.ListOfLanguagesByCodeResult
                    .Select(lang => $"{lang.sName} ({lang.sISOCode})")
                    .ToList();
            }
            catch (Exception ex)
        ...
        public async Task<string> ObtenerNombreDeIdioma(string codigoIdioma)
        {
            try
            {
                var cliente = new CountryInfoServiceSoapTypeClient(
